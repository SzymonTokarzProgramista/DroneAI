...
class FaceTracker:
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        self._head_yaw_gain = config.tracking_head_yaw_gain
        self._head_yaw_min_speed = config.tracking_head_yaw_min_speed
    ...
    def select_target(self, faces: list[RecognizedFace]) -> RecognizedFace | None:
        ...
        # Fallback: when configured target is absent, track the best recognized identity.
        if not candidates:
            candidates = [face for face in faces if face.label not in {"unknown", "embedding-error"}]
        if not candidates:
            return None
    ...
    def build_command(self, frame_width: int, target_face: RecognizedFace | None) -> TrackingCommand:
        ...
        if self._head_pose_enabled and target_face.head_pose_ready and target_face.head_yaw_deg is not None:
            ...
            if abs(head_yaw_error_deg) > self._head_yaw_deadband_deg:
                head_yaw_yaw_velocity = self._clamp_speed_with_minimum(
                    head_yaw_error_deg * self._head_yaw_gain,
                    self._head_yaw_min_speed,
                    self._max_yaw_speed,
                )
        ...
        # Keep the drone facing the person by blending center error and face yaw cues.
        yaw_velocity = self._clamp_speed(yaw_velocity + head_yaw_yaw_velocity, self._max_yaw_speed)
        ...
        if head_yaw_error_deg is not None and abs(head_yaw_error_deg) > self._head_yaw_deadband_deg:
